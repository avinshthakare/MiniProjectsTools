...
<#
.SYNOPSIS
    DocSnapper - screen capture to Word, plain PowerShell (no execution-policy override).

.DESCRIPTION
    This is a normal, unsigned .ps1 file. It does not attempt to bypass or change
    your machine's PowerShell execution policy in any way - if your organization's
    policy blocks unsigned scripts, this will fail with the standard Windows error
    ("running scripts is disabled on this system"), which is expected and correct.

.HOW TO RUN (pick whichever your IT policy allows)
    1. Right-click the file -> "Run with PowerShell"
       (respects whatever policy is already set on this machine - no override)
    2. From a PowerShell prompt:  .\ScreenCaptureToWord.ps1
    3. If Windows added a "downloaded from the internet" block (Zone.Identifier),
       remove just that flag with:  Unblock-File .\ScreenCaptureToWord.ps1
       This does not touch execution policy - it only clears the mark-of-the-web
       flag Windows adds to files from email/downloads.

.IF IT'S BLOCKED
    That means your org enforces RemoteSigned/AllSigned/Restricted or an
    AppLocker/WDAC rule - working as intended. The compliant ways through are:
      a) Ask IT to Authenticode-sign this script with your org's code-signing cert
      b) Ask IT to whitelist this script's path/hash in AppLocker or WDAC
      c) Ask IT whether Office's "automation security" GPO permits COM automation
         of Word from scripts (this app drives Word via COM, not VBA macros)
    Do not self-elevate around a policy block - that's the actual compliance risk.

.NOTES
    No network calls, no persistence (no registry Run key, no scheduled task,
    no auto-start), no admin rights required. Everything stays local to this
    machine and the Word document you target.
#>
...
$btnUndo.Add_Click({
    try {
        ...
        if ($doc.InlineShapes.Count -gt 0) {
            ...
            $lblStatus.Text = "Nothing to undo"
        }
    } catch {
        $lblStatus.Text = "Undo Err: $($_.Exception.Message)"
    }
})
...
$btnSaveDoc.Add_Click({
    try {
        ...
        $lblStatus.Text = "Save Err: $($_.Exception.Message)"
    }
})
